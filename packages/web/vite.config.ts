...
const pwaDevEnabled = process.env.OMPCHAMBER_DISABLE_PWA_DEV !== '1';
...
const themeJsonHmrPlugin = () => ({
  name: 'ompchamber-theme-json-hmr',
  handleHotUpdate({ file, server }: { file: string; server: { ws: { send: (payload: unknown) => void } } }) {
    ...
    try {
      server.ws.send({
        type: 'custom',
        event: 'ompchamber:theme-updated',
...
export default defineConfig({
  ...
  resolve: {
    alias: [
      { find: '@ompchamber/ui', replacement: path.resolve(__dirname, '../ui/src') },
      { find: '@ompchamber/agent-protocol', replacement: path.resolve(__dirname, '../agent-protocol/src') },
