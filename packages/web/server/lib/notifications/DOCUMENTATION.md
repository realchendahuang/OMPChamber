...
- Returned API:
  ...
  - `resolveApnsConfig()`
- Configuration (env first, then `settings.apnsConfig`): `OMPCHAMBER_APNS_KEY_ID`, `OMPCHAMBER_APNS_TEAM_ID`, `OMPCHAMBER_APNS_P8` (PEM contents; literal `\n` accepted) or `OMPCHAMBER_APNS_P8_PATH`, `OMPCHAMBER_APNS_BUNDLE_ID` (default `com.ompchamber.app`), `OMPCHAMBER_APNS_ENVIRONMENT` (optional override forcing every send to `sandbox` or `production`; when unset, each token is delivered to the environment it registered with, defaulting to `production` for tokens without one).
