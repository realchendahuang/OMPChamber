...
async function stopCommand(options) {
    ...
    if (options.explicitPort) {
      ...
      if (runningInstances.length === 0) {
        ...
        if (showOutput) {
          logStatus('info', `no OMPChamber instance found on port ${options.port}`);
      ...
      if (explicitInstance.runtime === 'desktop') {
        ...
        if (isJsonMode(options)) {
          printJson({ stoppedCount: 0, results: jsonResults, messages: [{ level: 'warning', code: 'DESKTOP_MANAGED_PORT', message: `Port ${options.port} is managed by OMPChamber Desktop and cannot be stopped with this command.` }] });
        }
        if (showOutput) {
          logStatus('warning', `port ${options.port} is managed by OMPChamber Desktop`, 'cannot be stopped with this command');
      ...
      if (explicitInstance.source === 'probe') {
        ...
        if (showOutput && !unmanagedStopSpin) {
          logStatus('info', `found unmanaged OMPChamber instance on port ${options.port}`, 'attempting shutdown');
        }
        unmanagedStopSpin?.start(`Stopping unmanaged OMPChamber on port ${options.port}...`);
        ...
        if (stopped) {
          unmanagedStopSpin?.stop(`Stopped unmanaged OMPChamber on port ${options.port}`);
          jsonResults.push({ port: options.port, runtime: 'unmanaged', stopped: true });
          if (isJsonMode(options)) {
            printJson({ stoppedCount: 1, results: jsonResults });
          }
          if (showOutput && !unmanagedStopSpin) {
            logStatus('success', `stopped OMPChamber on port ${options.port}`);
          ...
          unmanagedStopSpin?.error(`Could not stop OMPChamber on port ${options.port}`);
          ...
          if (isJsonMode(options)) {
            printJson({
              ...
              stoppedCount: 0,
              results: jsonResults,
              messages: [{ level: 'error', code: 'STOP_FAILED', message: `Could not stop OMPChamber on port ${options.port}.` }],
            });
          }
          if (showOutput && !unmanagedStopSpin) {
            logStatus('error', `could not stop OMPChamber on port ${options.port}`);
            finish('failed');
          }
          printQuietStopResults();
        }
      ...
      if (explicitInstance.source === 'registry-unconfirmed') {
        ...
        if (showOutput && !unconfirmedStopSpin) {
          logStatus('info', `found unconfirmed OMPChamber pid ${explicitInstance.pid} on port ${options.port}`, 'HTTP shutdown endpoint is unreachable; stopping by PID');
        }
        unconfirmedStopSpin?.start(`Stopping unconfirmed OMPChamber on port ${options.port}...`);
        ...
        if (stopped || !isProcessRunning(explicitInstance.pid)) {
          ...
          unconfirmedStopSpin?.stop(`Stopped OMPChamber PID ${explicitInstance.pid}`);
        ...
        unconfirmedStopSpin?.error(`Could not stop OMPChamber PID ${explicitInstance.pid}`);
        ...
        if (isJsonMode(options)) {
          printJson({
            ...
            stoppedCount: 0,
            results: jsonResults,
            messages: [{ level: 'error', code: 'STOP_FAILED', message: `Could not stop OMPChamber PID ${explicitInstance.pid}.` }],
          });
        }
        if (showOutput && !unconfirmedStopSpin) {
      ...
      if (showOutput) {
        logStatus('info', 'No running OMPChamber instances found');
        finish('nothing to stop');
      }
      printQuietStopResults();
      return;
    ...
    for (const instance of runningInstances) {
      ...
      stopSpin?.start(`Stopping OMPChamber on port ${instance.port}...`);
      try {
        ...
        stopSpin?.stop(`Stopped OMPChamber on port ${instance.port}`);
        jsonResults.push({ port: instance.port, pid: instance.pid, stopped: true });
        if (showOutput && !stopSpin) {
          logStatus('success', `stopped port ${instance.port}`);
        }
      } catch (error) {
        stopSpin?.error(`Failed to stop OMPChamber on port ${instance.port}`);
...
async function restartCommand(options, serveCommand) {
    ...
    if (runningInstances.length === 0) {
      ...
      if (showOutput) {
        logStatus('info', 'No running OMPChamber instances to restart');
    ...
    for (const instance of runningInstances) {
      if (instance.runtime === 'desktop') {
        const message = `Port ${instance.port} is managed by OMPChamber Desktop and cannot be restarted with this command.`;
        if (isJsonMode(options)) {
          printJson({
            status: 'warning',
            restartedCount: 0,
        ...
        if (showOutput) {
          logStatus('warning', `port ${instance.port} is managed by OMPChamber Desktop`, 'cannot be restarted with this command');
      ...
      restartSpin?.start(`Restarting OMPChamber on port ${instance.port}...`);
      try {
        await runStop({
          explicitPort: true,
          port: instance.port,
        ...
        restartSpin?.stop(`Restarted OMPChamber on port ${restartedPort}`);
        if (showOutput && !restartSpin) {
          logStatus('success', `port ${restartedPort} restarted`, `mode: ${launchMode}`);
        }
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        restartSpin?.error(`Failed to restart OMPChamber on port ${instance.port}`);
        if (showOutput && !restartSpin) {
