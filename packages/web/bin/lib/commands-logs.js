...
async function logsCommand(options) {
  ...
  if (options.all) {
    targets = running;
    if (targets.length === 0) {
      throw new Error('No running OMPChamber instance found.');
    }
  ...
  if (isJsonMode(options)) {
    if (options.follow) {
      throw new Error('`ompchamber logs --json` requires `--no-follow` for deterministic JSON output.');
    }
