...
This directory contains the non-entrypoint implementation for the OMPChamber CLI. `packages/web/bin/cli.js` should stay thin: it owns bootstrap, command wiring, top-level dispatch, signal/cancel handling, and compatibility exports. Domain logic belongs in these modules.
...
- `commands-status.js`
  - Implements `ompchamber status`.
  - Formats discovered instances and tunnel readiness/status for human, quiet, and JSON output.

- `commands-session.js`
  - Implements `ompchamber session create`, `send`, `fork`, `list`, `status`, and `messages`.
...
- `commands-connect-url.js`
  - Implements `ompchamber connect-url`.
...
- `commands-update.js`
  - Implements `ompchamber update`.
  - Loads the package-manager helper, performs update flow, and coordinates restart behavior after updates.

- `commands-tunnel.js`
  - Implements `ompchamber tunnel` and its subcommands: `profile`, `providers`, `ready`, `doctor`, `status`, `start`, `stop`, and `completion`.
...
- `cli-control.js`
  - Sends one typed action request to the authenticated OMPChamber control endpoint and maps HTTP failures to CLI exit behavior.
  - Must not reproduce session, scheduled-task, project-resolution, or wait orchestration.

- `cli-api-target.js`
  - Resolves the target OMPChamber runtime for control-plane commands, preferring desktop unless a port is explicit.
