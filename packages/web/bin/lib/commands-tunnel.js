...
async function resolveTargetInstance({
  ...
  if (options.all && requireAll) {
    if (running.length === 0) {
      throw new Error('No running OMPChamber instance found. Start one with `ompchamber serve`.');
    }
    return running;
  }
  ...
  if (options.explicitPort) {
      ...
      if (rejectDesktopRuntime) {
        ...
        if (!attachability.attachable) {
          if (attachability.reason === 'desktop') {
            throw new Error(
              `Port ${options.port} is used by OMPChamber Desktop app. Tunnel attach requires a CLI instance from \`ompchamber serve\`.`
            );
          }
          throw new Error(
            `Port ${options.port} is not an attachable OMPChamber tunnel instance. Ensure it is healthy and running OMPChamber CLI runtime.`
          );
        }
      }
      return found;
    ...
    throw new Error(`No running OMPChamber instance found on port ${options.port}.`);
  }
  ...
  if (rejectDesktopRuntime) {
    ...
    if (attachableEntries.length > 1) {
      ...
      throw new Error(`Multiple attachable OMPChamber instances found: ${ports}. Use --port <port> or --all.`);
    }
    ...
    if (sawDesktop) {
      throw new Error('Only OMPChamber Desktop instance(s) detected. Tunnel attach requires a CLI instance from `ompchamber serve`.');
    }

    throw new Error('No attachable OMPChamber instance found. Start one with `ompchamber serve`.');
  }
  ...
  throw new Error(`Multiple OMPChamber instances found: ${ports}. Use --port <port> or --all.`);
}
...
async function handleTunnelProfileSubcommand(options, action, { boldText, ensureTunnelProfilesMigrated }) {
  ...
  if (!sub) {
    ...
    if (!isQuietMode(options)) {
      ...
      clackLog.step('List profiles: `ompchamber tunnel profile list`');
      clackLog.step('Show one profile: `ompchamber tunnel profile show --name <name>`');
      clackLog.step('Add profile: `ompchamber tunnel profile add --provider cloudflare --mode managed-remote --name <name> --hostname <host> --token <token>`');
      clackLog.step('Remove profile: `ompchamber tunnel profile remove --name <name>`');
  ...
  if (sub === 'add') {
    ...
    logStatus('info', '[START_PROFILE]', `ompchamber tunnel start --profile ${added.name}`);
    clackOutro('');
    return;
  }
    ...
    switch (subcommand) {
      ...
      case 'ready': {
        ...
        for (const entry of entries) {
          try {
            const { response, body } = await requestJson(entry.port, `/api/ompchamber/tunnel/check?provider=${encodeURIComponent(provider)}`);
            if (!response.ok) {
      ...
      case 'status': {
        ...
        for (const entry of entries) {
          try {
            const { response, body } = await requestJson(entry.port, '/api/ompchamber/tunnel/status');
            if (!response.ok) {
      ...
      case 'doctor': {
          ...
          for (const diagnosticsEntry of diagnosticsEntries) {
            try {
              ...
              const { response, body } = await requestJson(
                ...
                `/api/ompchamber/tunnel/doctor?${query.toString()}`,
                doctorFetchOptions,
              );
        ...
        if (desktopUnavailablePorts.length > 0) {
          clackLog.message('Only CLI instances (ompchamber serve) support tunneling.');
        }

        if (cliPorts.length === 0 && unavailablePorts.length === 0) {
          logStatus('warning', 'No running instances found', 'Start one with `ompchamber serve`.');
          clackOutro('No ports available');
          return;
        }
        if (cliPorts.length === 0) {
          logStatus('warning', 'No CLI instances available for tunneling', 'Start one with `ompchamber serve`.');
      ...
      case 'start': {
        ...
        if (!options.explicitPort && canPrompt(options)) {
          ...
          if (runningInstances.length > 1) {
            ...
            if (safeInstances.length === 0) {
              throw new TunnelCliError(
                'All discovered OMPChamber instance ports are browser-unsafe. Start or target a safe port (3000, 5173, 8080, or high ephemeral).',
                EXIT_CODE.USAGE_ERROR,
              );
            }
            ...
            if (attachableSafeInstances.length === 0) {
              throw new TunnelCliError(
                'No attachable OMPChamber CLI instances found on safe ports. Start one with `ompchamber serve --port 3000`.',
                EXIT_CODE.USAGE_ERROR,
              );
            }

            const selectedPort = await clackSelect({
              message: 'Select OMPChamber instance port',
        ...
        if (instance?.autoStarted && shouldRenderHumanOutput(options)) {
          logStatus(
            ...
            `logs: ompchamber logs -p ${instance.port}`,
          );
        }
        ...
        if (instance?.autoStarted) {
          ...
          healthProgress?.start(`Waiting for OMPChamber on port ${instance.port} to become healthy (up to 60s)...`);
          ...
          const healthy = await waitForServerHealth(instance.port, {
            ...
            onTick({ elapsedMs, complete }) {
              ...
              if (delta > 0) {
                ...
                healthProgress.message(`Waiting for OMPChamber health (${progressedSeconds}s / 60s)...`);
              }
          ...
          if (!healthy) {
            healthProgress?.stop('OMPChamber is still starting');
            throw new Error(
              `OMPChamber on port ${instance.port} is still starting after 60s. Startup time can vary by machine performance. ` +
              `Wait another minute, then check health with \`curl -fsS ${buildLocalUrl(instance.port, '/health')}\`. ` +
              `If health is OK, retry tunnel start with \`ompchamber tunnel start --port ${instance.port}\`. ` +
              `For diagnostics run \`ompchamber logs -p ${instance.port}\`.`
            );
          }
        ...
        if (selectedProfile && mode === 'managed-remote') {
          ...
          const { response: presetResponse, body: presetBody } = await requestJson(instance.port, '/api/ompchamber/tunnel/managed-remote-token', {
            method: 'PUT',
        ...
        let body;
        try {
          ({ response, body } = await requestJson(instance.port, '/api/ompchamber/tunnel/start', {
            method: 'POST',
            body: JSON.stringify(payload),
            timeoutMs: 60000,
          }));
        } catch (error) {
          if (error instanceof Error && /\/api\/ompchamber\/tunnel\/start/.test(error.message) && /timed out/.test(error.message)) {
            spin?.error('Tunnel start timed out');
            throw new Error(
              `Tunnel start timed out after 60s. cloudflared may still be starting; check with \`ompchamber tunnel status --port ${instance.port}\`. Run \`ompchamber logs -p ${instance.port}\` for details.`
            );
          }
          spin?.error('Tunnel start failed');
          const message = error instanceof Error ? error.message : String(error);
          throw new Error(`${message} Run \`ompchamber logs -p ${instance.port}\` for details.`);
        }
        ...
        if (!response.ok || !body?.ok) {
          ...
          const userError = isCloudflareTimeout
            ...
            : baseError;
          throw new Error(`${userError} Run \`ompchamber logs -p ${instance.port}\` for details.`);
        }
        ...
        if (isJsonMode(options)) {
          ...
          const optionalTips = [
            { line: 'Check status', detail: 'ompchamber tunnel status' },
            { line: 'Stop tunnel', detail: 'ompchamber tunnel stop' },
          ...
          if (!selectedProfile && mode === 'managed-remote' && typeof hostname === 'string' && hostname.trim().length > 0) {
            ...
            optionalTips.push({ line: 'Start from saved profile', detail: 'ompchamber tunnel start --profile <name>' });
          }
      ...
      case 'stop': {
        ...
        for (const entry of entries) {
          ...
          try {
            const { response, body } = await requestJson(entry.port, '/api/ompchamber/tunnel/stop', {
              method: 'POST',
            });
            if (!response.ok) {
      ...
      default: {
        ...
        const hint = suggestion ? ` Did you mean '${suggestion}'?` : '';
        throw new TunnelCliError(
          `Unknown tunnel subcommand '${subcommand}'.${hint} Use 'ompchamber tunnel help'.`,
          EXIT_CODE.USAGE_ERROR
        );
      }
    }
