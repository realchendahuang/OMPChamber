...
async function resolveAvailablePort(desiredPort, explicitPort = false, onNotice) {
  ...
  if (occupant?.runtime === 'desktop') {
    message = `Port ${startPort} is used by OMPChamber Desktop; using a free port`;
  } else if (occupant?.runtime) {
    message = `Port ${startPort} is used by an existing OMPChamber instance; using a free port`;
  } else {
