import type { Message, Session } from '@ompchamber/agent-protocol/domain-types';
...
const sendPlainMessage = async (
  ...
  await optimisticSend({
    ...
    send: (messageID) => {
      assertAutoReviewRuntimeStillCurrent(expectedRuntimeKey);
      return agentClient.sendMessage({
        id: sessionID,
        directory,
        providerID: resolved.providerID,
        modelID: resolved.modelID,
...
const getSessionOrNull = async (sessionID: string, directory: string): Promise<Session | null> => {
  try {
    return await agentClient.getSession(sessionID, directory);
  } catch {
    return null;
  }
};
...
const createOrReuseReviewSession = async (originalSessionID: string, directory: string, expectedRuntimeKey?: string): Promise<Session> => {
  assertAutoReviewRuntimeStillCurrent(expectedRuntimeKey);
  const original = await agentClient.getSession(originalSessionID, directory);
  assertAutoReviewRuntimeStillCurrent(expectedRuntimeKey);
  ...
  assertAutoReviewRuntimeStillCurrent(expectedRuntimeKey);
  const review = await agentClient.createSession({
  ...
  try {
    ...
    assertAutoReviewRuntimeStillCurrent(expectedRuntimeKey);
    await agentClient.deleteSession(review.id, directory).catch((deleteError) => {
...
export const sendReviewFeedbackToOriginal = async (reviewSessionID: string, directory: string, reviewFeedback: string, expectedRuntimeKey?: string): Promise<string> => {
  assertAutoReviewRuntimeStillCurrent(expectedRuntimeKey);
  const reviewSession = await agentClient.getSession(reviewSessionID, directory);
...
export const sendImplementationResponseToReviewer = async (originalSessionID: string, directory: string, implementationResponse: string, autoReview = false, expectedRuntimeKey?: string): Promise<string> => {
  assertAutoReviewRuntimeStillCurrent(expectedRuntimeKey);
  const originalSession = await agentClient.getSession(originalSessionID, directory);
  ...
  try {
    reviewSession = await agentClient.getSession(reviewSessionID, directory);
  } catch (error) {
    assertAutoReviewRuntimeStillCurrent(expectedRuntimeKey);
