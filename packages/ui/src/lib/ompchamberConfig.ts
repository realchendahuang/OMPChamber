 ...
 * Stores per-project settings in ~/.config/ompchamber/<projectId>.json.
...
const LEGACY_CONFIG_FILENAME = 'openchamber.json';
...
interface OMPChamberConfig {
  ...
  projectNotes?: string;
  projectTodos?: OMPChamberProjectTodoItem[];
  projectPlanFiles?: OMPChamberProjectPlanFileLink[];
...
export interface OMPChamberProjectTodoItem {
  id: string;
  text: string;
  completed: boolean;
  createdAt: number;
...
export interface OMPChamberProjectPlanFileLink {
  id: string;
  path: string;
  createdAt: number;
}

export interface OMPChamberProjectPlanFile {
  title: string;
  body: string;
  raw: string;
  path: string;
...
export interface OMPChamberProjectNotesTodos {
  notes: string;
  todos: OMPChamberProjectTodoItem[];
}

export interface OMPChamberProjectContextData extends OMPChamberProjectNotesTodos {
  plans: OMPChamberProjectPlanFileLink[];
}

export const OMPCHAMBER_PROJECT_NOTES_MAX_LENGTH = 3000;
export const OMPCHAMBER_PROJECT_TODO_TEXT_MAX_LENGTH = 120;
...
const OMPCHAMBER_PROJECT_PLAN_TITLE_MAX_LENGTH = 160;
...
const getLegacyConfigPath = (projectDirectory: string): string => {
  return joinPath(joinPath(projectDirectory, LEGACY_CONFIG_DIR), LEGACY_CONFIG_FILENAME);
};
...
const sanitizeProjectNotes = (value: unknown): string => {
  ...
  return trimToMaxLength(value, OMPCHAMBER_PROJECT_NOTES_MAX_LENGTH);
};

const sanitizeProjectTodoItems = (value: unknown): OMPChamberProjectTodoItem[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: OMPChamberProjectTodoItem[] = [];
  for (const entry of value) {
    ...
    const text = trimToMaxLength(textRaw.trim(), OMPCHAMBER_PROJECT_TODO_TEXT_MAX_LENGTH);
    if (!id || !text) {
      continue;
    }
...
const sanitizeProjectPlanFileLinks = (value: unknown): OMPChamberProjectPlanFileLink[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: OMPChamberProjectPlanFileLink[] = [];
...
const sanitizeProjectNotesAndTodos = (value: {
  notes?: unknown;
  todos?: unknown;
} | null | undefined): OMPChamberProjectNotesTodos => {
  return {
...
const sanitizeProjectContextData = (value: {
  ...
  todos?: unknown;
  plans?: unknown;
} | null | undefined): OMPChamberProjectContextData => {
...
const sanitizePlanTitle = (value: string): string => {
  return trimToMaxLength(value.trim(), OMPCHAMBER_PROJECT_PLAN_TITLE_MAX_LENGTH);
};
...
async function readOMPChamberConfig(project: ProjectRef): Promise<OMPChamberConfig | null> {
  ...
  // LEGACY_PROJECT_CONFIG: migrate project-local ompchamber.json -> ~/.config/ompchamber/projects/<projectId>.json
  ...
  try {
    const wrote = await writeOMPChamberConfig(project, legacyConfig);
    if (wrote) {
      await deleteLegacyOMPChamberConfig(projectDirectory);
    }
  } catch {
...
export async function getProjectNotesAndTodos(project: ProjectRef): Promise<OMPChamberProjectNotesTodos> {
  const config = await readOMPChamberConfig(project);
...
export async function saveProjectNotesAndTodos(
  project: ProjectRef,
  value: OMPChamberProjectNotesTodos
): Promise<boolean> {
...
export async function getProjectContextData(project: ProjectRef): Promise<OMPChamberProjectContextData> {
  const config = await readOMPChamberConfig(project);
...
async function getProjectPlanFiles(project: ProjectRef): Promise<OMPChamberProjectPlanFileLink[]> {
  const config = await readOMPChamberConfig(project);
...
async function saveProjectPlanFiles(
  project: ProjectRef,
  value: OMPChamberProjectPlanFileLink[]
): Promise<boolean> {
...
export async function readProjectPlanFile(path: string): Promise<OMPChamberProjectPlanFile | null> {
...
): Promise<OMPChamberProjectPlanFileLink | null> {
...
async function deleteLegacyOMPChamberConfig(projectDirectory: string): Promise<void> {
  const legacyPath = getLegacyConfigPath(projectDirectory);
  const runtimeFiles = getRuntimeFilesAPI();
