...
export const DesktopNetworkSettings: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(t('settings.ompchamber.desktopNetwork.error.loadFailed'));
        }
        ...
        if (!cancelled) {
          setError(cause instanceof Error ? cause.message : t('settings.ompchamber.desktopNetwork.error.loadFailed'));
        }
      } finally {
        if (!cancelled) {
          setIsLoading(false);
  ...
  const handleLaunchAtLoginToggle = React.useCallback(async () => {
    ...
    try {
      ...
      if (!status?.supported) {
        throw new Error(t('settings.ompchamber.desktopNetwork.error.launchAtLoginUnsupported'));
      }
      setLaunchAtLoginEnabled(status.enabled);
    } catch (cause) {
      setLaunchAtLoginEnabled(!nextValue);
      setError(cause instanceof Error ? cause.message : t('settings.ompchamber.desktopNetwork.error.launchAtLoginSaveFailed'));
    } finally {
  ...
  const handleMinimizeToTrayToggle = React.useCallback(async () => {
    ...
    try {
      ...
      if (!status) {
        throw new Error(t('settings.ompchamber.desktopNetwork.error.minimizeToTraySaveFailed'));
      }
      if (!status.supported) {
        throw new Error(t('settings.ompchamber.desktopNetwork.error.minimizeToTrayUnsupported'));
      }
      setMinimizeToTrayEnabled(status.enabled);
    } catch (cause) {
      setMinimizeToTrayEnabled(!nextValue);
      setError(cause instanceof Error ? cause.message : t('settings.ompchamber.desktopNetwork.error.minimizeToTraySaveFailed'));
    } finally {
  ...
  const handleKeepAwakeToggle = React.useCallback(async () => {
    ...
    try {
      ...
      if (!status?.supported) {
        throw new Error(t('settings.ompchamber.desktopNetwork.error.keepAwakeUnsupported'));
      }
      setKeepAwakeEnabled(status.enabled);
    } catch (cause) {
      setKeepAwakeEnabled(!nextValue);
      setError(cause instanceof Error ? cause.message : t('settings.ompchamber.desktopNetwork.error.keepAwakeSaveFailed'));
    } finally {
  ...
  const handleSaveAndRestart = React.useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(t('settings.ompchamber.desktopNetwork.error.saveFailed'));
      }
      ...
      if (!restarted) {
        throw new Error(t('settings.ompchamber.desktopNetwork.error.savedRestartFailed'));
      }
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : t('settings.ompchamber.desktopNetwork.error.saveFailed'));
      setIsSaving(false);
    }
  ...
  return (
    <SettingsSection title={t('settings.ompchamber.desktopNetwork.title')}>
      <div className="space-y-3">
          ...
          <div className={SETTINGS_OPTION_STACK_CLASS}>
            {launchAtLoginSupported ? (
              <SettingsCheckboxRow
                ...
                label={t('settings.ompchamber.desktopNetwork.field.launchAtLogin')}
                info={t('settings.ompchamber.desktopNetwork.field.launchAtLoginDescription')}
                ariaLabel={t('settings.ompchamber.desktopNetwork.field.launchAtLoginAria')}
              />
            ) : null}
            ...
            {isMacDesktop ? (
              <SettingsCheckboxRow
                ...
                label={t('settings.ompchamber.desktopNetwork.field.macMenuBar')}
                info={t('settings.ompchamber.desktopNetwork.field.macMenuBarDescription')}
                ariaLabel={t('settings.ompchamber.desktopNetwork.field.macMenuBarAria')}
              />
            ) : null}
            ...
            {minimizeToTraySupported ? (
              <SettingsCheckboxRow
                ...
                label={t('settings.ompchamber.desktopNetwork.field.minimizeToTray')}
                info={t('settings.ompchamber.desktopNetwork.field.minimizeToTrayDescription')}
                ariaLabel={t('settings.ompchamber.desktopNetwork.field.minimizeToTrayAria')}
              />
            ) : null}
            ...
            {keepAwakeSupported ? (
              <SettingsCheckboxRow
                ...
                label={t('settings.ompchamber.desktopNetwork.field.keepAwake')}
                info={t('settings.ompchamber.desktopNetwork.field.keepAwakeDescription')}
                ariaLabel={t('settings.ompchamber.desktopNetwork.field.keepAwakeAria')}
              />
            ) : null}
          </div>
        ) : null}
        ...
        <SettingsStackedField
          ...
          label={(
            <label htmlFor="desktop-ui-password">
              {t('settings.ompchamber.desktopPassword.field.password')}
            </label>
          )}
          info={t('settings.ompchamber.desktopPassword.field.passwordDescription')}
        >
          <Input
            ...
            placeholder={t('settings.ompchamber.desktopPassword.field.passwordPlaceholder')}
            disabled={isLoading || isSaving}
            required={draftValue}
          ...
          <Button
            ...
            aria-label={t(showPassword ? 'settings.ompchamber.desktopPassword.actions.hidePassword' : 'settings.ompchamber.desktopPassword.actions.showPassword')}
        ...
        <div className={SETTINGS_OPTION_STACK_CLASS}>
          <SettingsCheckboxRow
            ...
            disabled={isLoading || isSaving}
            label={t('settings.ompchamber.desktopNetwork.field.allowLanAccess')}
            info={t('settings.ompchamber.desktopNetwork.field.allowLanAccessDescription')}
            description={(
              <>
                <span className="block text-[var(--status-warning)]/85">
                  {t('settings.ompchamber.desktopNetwork.field.warning')}
                </span>
                {lanRequiresPassword || lanBlockedByMissingPassword ? (
                  <span className="block text-[var(--status-warning)]/85">
                    {t('settings.ompchamber.desktopNetwork.field.passwordRequiredWarning')}
                  </span>
                ) : null}
              </>
            )}
            ariaLabel={t('settings.ompchamber.desktopNetwork.field.allowLanAccessAria')}
          />
        </div>
        ...
        {lanUrl ? (
          <div className="typography-micro text-muted-foreground/80">
            {isDirty && !savedValue
              ? t('settings.ompchamber.desktopNetwork.hint.openAfterRestart')
              : t('settings.ompchamber.desktopNetwork.hint.openNow')}
        ...
        <div className="flex justify-start py-1.5">
          ...
          >
            {isSaving ? t('settings.common.actions.saving') : t('settings.ompchamber.desktopNetwork.actions.saveAndRestart')}
          </Button>
        </div>
      </div>
    </SettingsSection>
