...
export const KeyboardShortcutsSettings: React.FC = () => {
  ...
  const actionLabel = React.useCallback((id: string, fallbackLabel: string): string => {
    const key = `settings.ompchamber.keyboardShortcuts.action.${id}.label`;
  ...
  const saveCombo = React.useCallback((actionId: string, combo: ShortcutCombo) => {
    ...
    setErrorText('');
    setWarningText(isRiskyBrowserShortcut(normalized) ? t('settings.ompchamber.keyboardShortcuts.warning.riskyBrowserShortcut') : '');
  ...
  const confirmOverwrite = React.useCallback(() => {
    ...
    setPendingOverwrite(null);
    setErrorText('');
    setWarningText(isRiskyBrowserShortcut(pendingOverwrite.combo) ? t('settings.ompchamber.keyboardShortcuts.warning.riskyBrowserShortcut') : '');
    setDraftByAction((current) => {
      const rest = { ...current };
  ...
  return (
    <SettingsSection
      ...
      title={t('settings.ompchamber.keyboardShortcuts.title')}
      divider={false}
      info={t('settings.ompchamber.keyboardShortcuts.tooltip')}
      headerAction={(
        <Button
          type="button"
          variant="outline"
          ...
          {pendingOverwrite && (
            <div className="rounded-lg border border-[var(--status-warning-border)] bg-[var(--status-warning-background)] p-3 flex flex-col @xl:flex-row @xl:items-center justify-between gap-3">
              <span className="typography-meta text-foreground">
                {t('settings.ompchamber.keyboardShortcuts.overwritePrompt')}
              </span>
              <div className="flex gap-2 shrink-0">
                <Button type="button" size="xs" className="!font-normal" onClick={confirmOverwrite}>{t('settings.ompchamber.keyboardShortcuts.actions.overwrite')}</Button>
      ...
      <div>
        {actions.map((action, index) => {
          ...
          const displayValue = capturingActionId === action.id
            ? t('settings.ompchamber.keyboardShortcuts.field.pressKeys')
            : isSurfaceSwitch && !isUnassignedDisplay
              ? `${formatShortcutForDisplay(displayCombo)}${t('settings.ompchamber.keyboardShortcuts.action.switch_context_surface.suffix')}`
              ...
              >
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (!next) {
                      setErrorText(t('settings.ompchamber.keyboardShortcuts.error.captureFirst'));
                      return;
                    }
