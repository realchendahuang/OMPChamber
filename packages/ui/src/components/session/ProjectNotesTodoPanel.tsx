...
import {
  ...
  OMPCHAMBER_PROJECT_NOTES_MAX_LENGTH,
  readProjectPlanFile,
  OMPCHAMBER_PROJECT_TODO_TEXT_MAX_LENGTH,
  saveProjectNotesAndTodos,
  type OMPChamberProjectPlanFileLink,
  type OMPChamberProjectTodoItem,
  type ProjectRef,
...
type ProjectPlanListItem = OMPChamberProjectPlanFileLink & {
  title: string;
};

const toPlanListItem = async (
  plan: OMPChamberProjectPlanFileLink,
...
const sortTodosWithCompletedLast = (items: OMPChamberProjectTodoItem[]): OMPChamberProjectTodoItem[] => [
  ...items.filter((todo) => !todo.completed),
  ...items.filter((todo) => todo.completed),
];

const insertTodoBeforeCompleted = (items: OMPChamberProjectTodoItem[], item: OMPChamberProjectTodoItem): OMPChamberProjectTodoItem[] => {
...
export const ProjectNotesTodoPanel: React.FC<ProjectNotesTodoPanelProps> = ({
  ...
  const [todos, setTodos] = React.useState<OMPChamberProjectTodoItem[]>([]);
  ...
  const persistProjectData = React.useCallback(
    async (nextNotes: string, nextTodos: OMPChamberProjectTodoItem[]) => {
      if (!projectRef) {
        return false;
      }
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('ompchamber:project-plan-saved', handleProjectContextRefresh);
    window.addEventListener('ompchamber:project-notes-updated', handleProjectContextRefresh);
    return () => {
      window.removeEventListener('ompchamber:project-plan-saved', handleProjectContextRefresh);
      window.removeEventListener('ompchamber:project-notes-updated', handleProjectContextRefresh);
    };
  }, [projectRef]);
  ...
  const handleAddTodo = React.useCallback(() => {
    ...
    const nextTodos = insertTodoBeforeCompleted(todos, {
      id: createTodoId(),
      text: trimmed.slice(0, OMPCHAMBER_PROJECT_TODO_TEXT_MAX_LENGTH),
      completed: false,
      createdAt: Date.now(),
    });
    setTodos(nextTodos);
  ...
  const todoInputValue = newTodoText.slice(0, OMPCHAMBER_PROJECT_TODO_TEXT_MAX_LENGTH);
  ...
  const handleDeletePlan = React.useCallback(
    async (planId: string) => {
      ...
      try {
        ...
        window.dispatchEvent(new CustomEvent('ompchamber:project-plan-saved', {
    ...
    <div className={cn('w-full min-w-0 space-y-3 p-3', className)}>
      <div className="space-y-1">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="typography-meta text-muted-foreground">{notes.length}/{OMPCHAMBER_PROJECT_NOTES_MAX_LENGTH}</span>
        </div>
        <Textarea
          value={notes}
          onChange={(event) => setNotes(event.target.value.slice(0, OMPCHAMBER_PROJECT_NOTES_MAX_LENGTH))}
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="typography-meta text-muted-foreground">{todoInputValue.length}/{OMPCHAMBER_PROJECT_TODO_TEXT_MAX_LENGTH}</span>
        </div>

        <div className="flex items-center gap-1.5">
          <Input
            value={todoInputValue}
            onChange={(event) => setNewTodoText(event.target.value.slice(0, OMPCHAMBER_PROJECT_TODO_TEXT_MAX_LENGTH))}
            onKeyDown={(event) => {
              if (event.key === 'Enter') {
                event.preventDefault();
                handleAddTodo();
