...
describe("attachment file preparation", () => {
  ...
  test("redacts credentials and omits bodies from HAR files", async () => {
    ...
    expect(entry.request.postData.text).toBe("[OMITTED BY OMPCHAMBER]")
    expect(entry.response.headers[0].value).toBe("[REDACTED]")
    expect(entry.response.content.text).toBe("[OMITTED BY OMPCHAMBER]")
    expect(entry.response.content.encoding).toBe("[OMITTED BY OMPCHAMBER]")
