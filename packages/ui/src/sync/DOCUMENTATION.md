...
- A failed bootstrap is classified as `os-permission` only when the owning runtime filesystem API independently confirms `EPERM`/`EACCES` for that exact directory. OMP/proxy error text is never used as permission evidence. The scheduler retains the directory-scoped reason so local Desktop can offer native folder selection before a forced retry.
...
Session loading instrumentation is disabled by default. Set `localStorage.ompchamber_session_load_perf` to `"1"`, reproduce the interaction, then inspect `window.__ompchamberSessionLoadPerformance.events`.
...
High-frequency sync diagnostics are separately disabled by default. Set `localStorage.ompchamber_sync_perf` to `"1"` before reload to enable fixed numeric counters for pipeline traffic, reducer publications, streaming reconciliations, entries/messages visited, targeted heartbeat work, and persistence serialization/write volume. The hot path performs only a null check while disabled; counters never retain IDs, payloads, or user content.

Browser profiling also enables `localStorage.ompchamber_stream_perf` to capture bounded aggregate timings and render counts for chat projections, message components, and major sidebar boundaries. These metrics contain no session IDs or user content and are reset immediately before each recording.
...
5. A disagreement between sources is logged once per session, and `__ompDebug.diagnoseSessionDirectory()` reports every source in precedence order.
...
so `0` reads as active in the UI, the event reducer, and the OMP app/TUI.
