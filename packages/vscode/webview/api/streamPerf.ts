...
declare global {
  interface Window {
    __ompchamberVsCodeStreamPerfState__?: StreamPerfState;
  }
}
...
const ensurePerfState = (): StreamPerfState | null => {
  ...
  if (!window.__ompchamberVsCodeStreamPerfState__) {
    const startedAt = Date.now();
    window.__ompchamberVsCodeStreamPerfState__ = {
  ...
  return window.__ompchamberVsCodeStreamPerfState__;
};
