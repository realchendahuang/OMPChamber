---
title: Dostęp zdalny
description: Napraw tunele, zdalne instancje i dostęp do OMPChamber z innego urządzenia.
---

# Dostęp zdalny

Gdy nie możesz dotrzeć do OMPChamber z telefonu lub innej maszyny, rozwiązanie zależy od tego, jak się łączysz.

## Najpierw sprawdź podstawy

- najpierw otwórz `http://localhost:3000` na tym samym komputerze — jeśli to zawiedzie, to nie jest problem zdalny; zobacz [Połączenie z OMP](/pl/troubleshooting/omp-connection/)
- potwierdź, że serwer działa, za pomocą `ompchamber status`
...
- przy parowaniu **Wszędzie** sprawdź **Settings → Remote Instances → OMPChamber Relay** na serwerze: status powinien brzmieć **Połączono**; jeśli nie, wyłącz i włącz relay ponownie
...
- uruchom `ompchamber tunnel status --all`
...
Gdy [zdalna instancja](/pl/remote-instances/) się zatnie, OMPChamber wskazuje krok, który zawiódł:

- **auth** — Twoje hasło SSH lub UI zostało odrzucone; wpisz je ponownie
- **install / start** — OMPChamber nie mógł skonfigurować lub uruchomić serwera na zdalnej maszynie; sprawdź wymagania tej maszyny
- **forwarding** — połączenie działa, ale port do Ciebie nie dociera; spróbuj innego portu lokalnego

## Za własnym serwerem

Jeśli umieściłeś OMPChamber za reverse proxy i ładuje się dziwnie lub nie chce się połączyć, zobacz [Reverse Proxy](/pl/reverse-proxy/).
