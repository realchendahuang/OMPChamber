...
description: Sparuj telefon, komputer lub inną przeglądarkę ze swoim serwerem OMPChamber za pomocą jednorazowego kodu QR.
---

# Połącz urządzenie

Sparuj inne urządzenie — aplikację mobilną, aplikację desktopową lub przeglądarkę na innej maszynie — ze swoim serwerem OMPChamber, skanując jednorazowy kod QR. To zalecany sposób łączenia urządzeń: nie ma portów do otwierania ani adresów do wpisywania.

## Sparuj urządzenie

1. Na maszynie z uruchomionym OMPChamber otwórz **Settings → Remote Instances → Połącz z tym serwerem** i naciśnij **Dodaj urządzenie**.
...
5. Na drugim urządzeniu zeskanuj kod:
   ...
   - **aplikacja desktopowa** — zamiast tego skopiuj link połączenia i wklej go w **Settings → Remote Instances → Inne serwery OMPChamber → Importuj link**
