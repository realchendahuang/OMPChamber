...
OMPChamber daje dostęp do Twojej maszyny i Twojego kodu, więc zabezpiecz go, zanim ktokolwiek poza Tobą będzie mógł do niego dotrzeć. Ta strona omawia hasło UI, klucze dostępu (passkeys) oraz to, co warto wiedzieć przed udostępnieniem OMPChamber w sieci.

## Ustaw hasło UI

Uruchom OMPChamber z hasłem, a interfejs webowy poprosi o nie:

```bash
ompchamber --ui-password be-creative-here
```

Możesz też ustawić je za pomocą zmiennej środowiskowej `OMPCHAMBER_UI_PASSWORD`, zamiast podawać je w wierszu poleceń. Po zalogowaniu OMPChamber zapamiętuje urządzenie na pewien czas, więc nie pyta za każdym razem.
...
Po ustawieniu hasła możesz dodać klucze dostępu (Face ID, Touch ID, klucz bezpieczeństwa) dla szybszego logowania. Dodaj je w **Settings → OMPChamber → Passkeys**.
...
- Domyślnie OMPChamber nasłuchuje tylko na Twojej własnej maszynie (`127.0.0.1`). Nasłuchiwanie szerzej wymaga celowej zmiany i najpierw powinieneś ustawić hasło.
...
- Jeśli umieszczasz OMPChamber za własnym serwerem HTTPS, zobacz [Reverse Proxy](/pl/reverse-proxy/).
...
- [Reverse Proxy](/pl/reverse-proxy/) — uruchom OMPChamber za własnym serwerem
