---
title: Reverse proxy
description: Skonfiguruj OMPChamber poprawnie za Nginx, Nginx Proxy Manager lub innym reverse proxy.
---

# Reverse proxy

Skorzystaj z tej strony, jeśli uruchamiasz OMPChamber za Nginx, Nginx Proxy Manager, Caddy, Cloudflare lub innym reverse proxy.

## Zanim ustawisz proxy

1. Najpierw potwierdź, że OMPChamber działa bezpośrednio.
...
- Wyłącz gzip na proxy, jeśli OMPChamber już kompresuje odpowiedzi.
...
- OMPChamber dostępny bezpośrednio w LAN
...
- OMPChamber kompresuje odpowiedzi HTTP za pomocą gzip (próg 1 KB).
...
Trasy strumieniowania SSE są wyłączone z kompresji przez OMPChamber, ale CDN może je nadal buforować. Sprawdź dokumentację swojego CDN, jak wyłączyć buforowanie na ścieżkach SSE.
