---
title: Remote Access
description: Fix tunnels, remote instances, and reaching OMPChamber from another device.
---

# Remote Access

When you can't reach OMPChamber from your phone or another machine, the fix depends on how you're connecting.

## Check the basics first

- open `http://localhost:3000` on the same computer first — if that fails, it's not a remote problem; see [OMP connection](/troubleshooting/omp-connection/)
- confirm the server is running with `ompchamber status`
...
- for **Anywhere** pairing, check **Settings → Remote Instances → OMPChamber Relay** on the server: it should say **Connected**; if not, disable and re-enable it
...
- run `ompchamber tunnel status --all`
...
When a [remote instance](/remote-instances/) stalls, OMPChamber names the step that failed:

- **auth** — your SSH or UI password was rejected; re-enter it
- **install / start** — OMPChamber couldn't set up or start the server on the remote machine; check that machine's requirements
- **forwarding** — the connection is up but the port isn't reaching you; try a different local port

## Behind your own server

If you put OMPChamber behind a reverse proxy and it loads oddly or won't connect, see [Reverse Proxy](/reverse-proxy/).
