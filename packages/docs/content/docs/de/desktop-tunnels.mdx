...
Die Desktop-App kann unter **Settings → OMPChamber → Tunnel** einen öffentlichen Tunnel erstellen. Für diesen Weg musst du OMPChamber nicht per CLI starten.

## Anbieter installieren

OMPChamber startet das Provider-CLI auf deinem Rechner. Installiere zuerst den Anbieter, den du verwenden möchtest:
...
1. Öffne **Settings → OMPChamber → Tunnel**.
