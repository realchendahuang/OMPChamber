...
4. Save — OMPChamber writes the provider block to OMP config and stores the key in OMP auth (literal keys) or records an `{env:VAR}` reference.
...
Provider sign-ins are stored by OMP, not OMPChamber, so they're shared with the `omp` CLI. If you set the same provider in more than one place, the most specific setting wins: a per-project setting overrides your personal one.
