...
Rendering and hosting still happen in `ompchamber-website` (`apps/docs`).
...
- creates `ompchamber-docs-source-<sha>.tar.gz`
...
to `ompchamber-website` so it re-syncs and redeploys the docs. This fires on
...
- `OMPCHAMBER_WEBSITE_REPO_TOKEN` — a token with `contents: write` (classic
  ...
  `realchendahuang/OMPChamber-website`. **Without it the dispatch step is skipped**
...
- `source_repo`
- `source_ref` — the ref the website checks out from `ompchamber` (`main` on a
  push, the tag on a release)
- `archive_name`

`ompchamber-website`'s `deploy-docs.yml` listens for this event
(`repository_dispatch: types: [docs_source_updated]`), checks out
`ompchamber` at `source_ref`, runs `docs:sync`, builds `apps/docs`, and deploys
