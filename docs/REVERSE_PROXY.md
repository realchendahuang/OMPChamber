...
Use this guide when running OMPChamber behind Nginx, Nginx Proxy Manager, Caddy, Cloudflare, or another reverse proxy.

## Before you proxy it

1. Confirm OMPChamber works directly first.
...
- Disable gzip on the proxy if OMPChamber is already compressing responses.
...
- OMPChamber reachable directly on LAN
...
location ~ ^/api/(event|global/event|notifications/stream|ompchamber/events)$ {
    proxy_pass http://127.0.0.1:3000;
...
- OMPChamber compresses HTTP responses with gzip (threshold 1 KB).
...
SSE streaming routes are excluded from compression by OMPChamber, but the CDN may still buffer them. Check your CDN documentation for how to disable buffering on SSE paths.
