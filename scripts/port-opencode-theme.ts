...
/*
 * Port OpenCode themes into full OMPChamber theme JSON files.
 *
 * Usage:
 *   bun run themes:port:opencode --list
 *   bun run themes:port:opencode github cursor lucent-orng --force
 *   bun run themes:port:opencode aura --out-dir /tmp/ompchamber-theme-port-test --force
 ...
 *     or existing OMPChamber theme JSON files
 *   - writes complete OMPChamber light/dark theme JSON output with solid core
...
type ThemeSource = DesktopTheme | ContextThemeJson | OMPChamberTheme;
...
function isOMPChamberTheme(value: ThemeSource): value is OMPChamberTheme {
...
function syntheticTokensFromOMPChamberTheme(theme: OMPChamberTheme): ResolvedTheme {
...
async function main(): Promise<void> {
  ...
  const generated: Array<{ fileName: string; theme: OMPChamberTheme }> = [];
    ...
    if (isOMPChamberTheme(source)) {
      ...
      generated.push({
        ...
        theme: buildTheme(
          syntheticTheme,
          variant,
          syntheticTokensFromOMPChamberTheme(source),
